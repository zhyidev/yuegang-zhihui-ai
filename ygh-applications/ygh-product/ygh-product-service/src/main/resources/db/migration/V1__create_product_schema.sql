...
(
    ...
    INDEX idx_product_category_parent (parent_id, sort_order)
);
...
(
    ...
    version  BIGINT UNSIGNED                                   NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
    UNIQUE KEY uk_product_brand_code (code)
);
...
(
    ...
    traceability_code VARCHAR(128)                                      NULL,
    status            VARCHAR(16) CHARACTER SET ascii COLLATE ascii_bin NOT NULL DEFAULT 'DRAFT',
    version           BIGINT UNSIGNED                                   NOT NULL DEFAULT 0,
    created_at        DATETIME(6)                                       NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    updated_at        DATETIME(6)                                       NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
    ...
    INDEX idx_product_sku_spu_status (spu_id, status),
...
(
    ...
    sort_order INT          NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
    INDEX idx_product_image_spu_sort (spu_id, sort_order),
    CONSTRAINT fk_product_image_spu FOREIGN KEY (spu_id) REFERENCES product_spu (id) ON DELETE CASCADE,
    CONSTRAINT fk_product_image_sku FOREIGN KEY (sku_id) REFERENCES product_sku (id) ON DELETE CASCADE
);
CREATE TABLE product_trace_event
(
    id            BIGINT                                            NOT NULL,
    ...
    PRIMARY KEY (id),
    INDEX idx_product_trace_sku_time (sku_id, occurred_at),
    CONSTRAINT fk_product_trace_sku FOREIGN KEY (sku_id) REFERENCES product_sku (id)
);
