...
(
    ...
    trace_description TEXT                                              NULL,
    created_at        DATETIME(6)                                       NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    PRIMARY KEY (id),
    UNIQUE KEY uk_product_batch_sku_no (sku_id, batch_no),
    INDEX idx_product_batch_expiry (expires_on),
    CONSTRAINT fk_product_batch_sku FOREIGN KEY (sku_id) REFERENCES product_sku (id) ON DELETE CASCADE
);
...
(
    ...
    PRIMARY KEY (id),
    INDEX idx_product_price_sku_time (sku_id, changed_at)
);
...
(
    ...
    created_at    DATETIME(6)                                       NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    PRIMARY KEY (id),
    INDEX idx_product_outbox_dispatch (status, next_retry_at)
);
