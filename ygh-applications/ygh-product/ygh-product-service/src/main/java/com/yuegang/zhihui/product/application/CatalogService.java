...
public final class CatalogService { // 定义目录服务最终类
    ...
    public CategoryView createCategory(SaveCategoryRequest r) { // 创建产品分类
        ...
        jdbc.update("INSERT INTO product_category(id,parent_id,code,name,sort_order) VALUES(?,?,?,?,?)", id, optional(r.parentId()), r.code(), r.name(), r.sortOrder());
        return category(id); // 返回新创建的分类视图
    }

    public List<CategoryView> categories(boolean enabled) { // 获取分类列表
        return jdbc.query("SELECT * FROM product_category" + (enabled ? " WHERE enabled=TRUE" : "") + " ORDER BY parent_id,sort_order,id", (ResultSet r, int n) -> mapCategory(r)); // 映射结果集到视图
    }

    public BrandView createBrand(SaveBrandRequest r) { // 创建品牌
        long id = next(); // 生成新 ID
        // 执行插入语句：分类 ID、父 ID、编码、名称，排序权重
        jdbc.update("INSERT INTO product_brand(id,code,name,logo_url) VALUES(?,?,?,?)", id, r.code(), r.name(), r.logoUrl());
        return brand(id); // 返回新品牌视图
    }

    public List<BrandView> brands(boolean enabled) { // 获取品牌列表
        return jdbc.query("SELECT * FROM product_brand" + (enabled ? " WHERE enabled=TRUE" : "") + " ORDER BY name,id", (ResultSet r, int n) -> mapBrand(r)); // 映射结果集到视图
    }
    ...
    public TraceEventView addTrace(String sku, TraceEventRequest r) { // 添加到溯源事件
        ...
        jdbc.update("INSERT INTO product_trace_event(id,sku_id,event_type,location_name,occurred_at,details_json) VALUES(?,?,?,?,?,?)", id, skuId, r.type(), r.location(), Timestamp.from(r.occurredAt().toInstant()), write(r.details()));
        // 返回新创建的溯源事件视图
        //return new TraceEventView(Long.toString(id), sku, r.type(), r.location(), r.occurredAt(), r.details());
        return null;
    }
    ...
    private Map<String, Object> read(String x) { // 将 JSON 字符串解析为 Map
        try {
            return x == null ? Map.of() : json.readValue(x, new TypeReference<Map<String, Object>>() {
            }); // 反序列化
        } catch (Exception e) {
