...
public class ProductAdministrationService { // 定义产品管理服务类
    ...
    public ProductAdministrationService(
            DataSource dataSource, ProductService products, ObjectMapper json) {
    ...
    public ProductBatchView batch(String sku, SaveProductBatchRequest command) {
        ...
        // 校验逻辑：过期日期不能再生产日期之前
        if (command.producedOn() != null
                && command.expiresOn() != null
                && command.expiresOn().isBefore(command.producedOn()))
            throw new BusinessException(ErrorCode.VALIDATION_ERROR);
    ...
    public ProductView update(String sku, UpdateProductRequest command) {
        ...
        Object[] before =
                jdbc.query(
                        "SELECT s.price,s.spu_id FROM product_sku s WHERE s.id=?",
                        result -> {
                            if (!result.next())
                                throw new BusinessException(
                                        ErrorCode.RESOURCE_NOT_FOUND); // 未找到抛出异常
                            return new Object[] {
                                result.getBigDecimal(1), result.getLong(2)
                            }; // 返回价格和 SPU ID
                        },
                        skuId);
        ...
        event(
                skuId,
                "PRODUCT_UPDATED",
                Map.of("skuId", sku, "price", command.price(), "currency", command.currency()));
        products.searchJob(skuId); // 触发异步搜索索引更新服务
        return products.get(sku, false); // 返回最新的产品详情视图
    }

    public List<ProductBatchView> batches(String sku) { // 查询至 SKU 的所有历史批次
        return jdbc.query(
                ...
                (result, row) ->
                        new ProductBatchView(
                                Long.toString(result.getLong(1)),
                                Long.toString(result.getLong(2)),
                                result.getString(3),
                                result.getString(4),
                                result.getString(5),
                                date(result.getDate(6)),
                                date(result.getDate(7)),
                                result.getString(8)),
                id(sku));
    }
