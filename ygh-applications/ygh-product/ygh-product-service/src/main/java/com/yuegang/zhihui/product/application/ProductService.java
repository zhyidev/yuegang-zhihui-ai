...
public final class ProductService { // 定义产品核心服务类，使用 final 防止被继承
    ...
    public List<ProductView> list(String category, String keyword, BigDecimal minimumPrice, BigDecimal maximumPrice,
        ...
        if (publicOnly) sql.append(" AND s.status='PUBLISHED' AND p.status='PUBLISHED'"); // 若仅公开数据，限定状态为 'PUBLISHED'
        ...
        if (origin != null && !origin.isBlank()) { // 根据原产地过滤（通过 EXISTS 子句检查关联的批次表）
            sql.append(" AND EXISTS(SELECT 1 FROM product_batch pb WHERE pb.sku_id=s.id AND pb.origin=?)");
            arguments.add(id(category));
        }
    ...
    void searchJob(long skuId) {
        jdbc.update("INSERT INTO product_search_job(id,sku_id) VALUES(?,?)", UUID.randomUUID().toString(), skuId);
    }
