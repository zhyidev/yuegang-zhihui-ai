...
public final class WalletAdminVerifier {
    ...
    public WalletAdminVerifier(@Value("${ygh.internal-request.hmac-base64}") String encoded) {
        byte[] secret = Base64.getDecoder().decode(encoded);
        try {
            signatures = new InternalUserContextSignature(secret, Clock.systemUTC(), Duration.ofSeconds(30));
        } finally {
    ...
    public void require(HttpServletRequest request) {
        try {
            ...
            if (!roles.contains("ADMIN") && !permissions.contains("wallet:read"))
