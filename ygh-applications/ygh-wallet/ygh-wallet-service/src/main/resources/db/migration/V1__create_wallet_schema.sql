...
(
    ...
    PRIMARY KEY (user_id),
    CONSTRAINT chk_wallet_balances CHECK (available_balance >= 0 AND frozen_balance >= 0)
);
...
(
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uk_wallet_transaction_request (request_id),
    INDEX idx_wallet_transaction_user_time (user_id, created_at),
    INDEX idx_wallet_transaction_reference (reference_id, type),
    CONSTRAINT fk_wallet_transaction_account FOREIGN KEY (user_id) REFERENCES wallet_account (user_id),
...
(
    ...
    PRIMARY KEY (id),
    INDEX idx_wallet_outbox_dispatch (status, next_retry_at)
);
CREATE TABLE wallet_inbox
(
