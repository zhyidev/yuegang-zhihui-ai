    ...
    void verifiesServiceAndTrustedUserMetadata() {
        ...
        var serviceMetadata =
                new InternalServiceSignature.Metadata(
                        "ygh-auth-service", "GET", serviceRequest.getRequestURI(), now);
        ...
        serviceRequest.addHeader(
                "X-YGH-Service-Signature", serviceSignatures.sign(serviceMetadata));
        var verifier = new InternalServiceVerifier(SECRET, Clock.systemUTC());
        assertThatCode(() -> verifier.verify(serviceRequest, "ygh-auth-service"))
                .doesNotThrowAnyException();
        assertThatThrownBy(() -> verifier.verify(serviceRequest, "other-service"))
                .isInstanceOf(BusinessException.class);
        serviceRequest.removeHeader("X-YGH-Service-Signature");
        assertThatThrownBy(() -> verifier.verify(serviceRequest, "ygh-auth-service"))
                .isInstanceOf(BusinessException.class);
        ...
        var userMetadata =
                new InternalUserContextSignature.Metadata(
                        ...
                        now);
        var userSignatures =
                new InternalUserContextSignature(SECRET, Clock.systemUTC(), Duration.ofSeconds(30));
        ...
        assertThatThrownBy(() -> resolver.resolve(userRequest))
                .isInstanceOf(BusinessException.class);
    }
}
