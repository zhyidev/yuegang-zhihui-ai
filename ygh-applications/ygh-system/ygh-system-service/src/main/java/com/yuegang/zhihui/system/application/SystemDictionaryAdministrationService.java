...
public class SystemDictionaryAdministrationService { // 定义类:系统字典管理服务
    ...
    public List<DictionaryAdminView> all() { // 方法:查询全量字典配置(后台管理用)
        // 查询并递归映射
        return jdbc.query(
                "SELECT id,code,name,enabled,version FROM system_dictionary_type ORDER BY code",
                (r, n) -> view(r));
    }
    ...
    public DictionaryAdminView saveType(SaveDictionaryTypeRequest c) { // 方法:保存字典类型
        ...
        int changed =
                c.version() == 0
                        ? jdbc.update(
                                "INSERT INTO system_dictionary_type(id,code,name,enabled) VALUES(?,?,?,?) ON DUPLICATE KEY UPDATE name=VALUES(name),enabled=VALUES(enabled) , version=version+1",
                                next(),
                                c.code(),
                                c.name(),
                                c.enabled())
                        : jdbc.update(
                                "UPDATE system_dictionary_type SET name=?,enabled=?,version=version+1 WHERE code=? AND version=?",
                                c.name(),
                                c.enabled(),
                                c.code(),
                                c.version());
    ...
    public DictionaryAdminView saveItem(String code, SaveDictionaryItemRequest c) {
        Long type =
                jdbc.query(
                        "SELECT id FROM system_dictionary_type WHERE code=?",
                        r -> r.next() ? r.getLong(1) : null,
                        code); // 先查找所属分类的ID
        ...
        int changed =
                c.version() == 0
                        ? jdbc.update(
                                "INSERT INTO system_dictionary_item(id,type_id,item_key,item_value,sort_order,enabled) VALUES(?,?,?,?,?,?) ON DUPLICATE KEY UPDATE item_value=VALUES(item_value),sort_order=VALUES(sort_order),enabled=VALUES(enabled),version=version+1",
                                next(),
                                type,
                                c.key(),
                                c.value(),
                                ...
                                c.enabled())
                        : jdbc.update(
                                "UPDATE system_dictionary_item SET item_value=?,sort_order=?,enabled=?,version= version+1 WHERE type_id=? AND item_key=? AND version=?",
                                c.value(),
                                c.sortOrder(),
                                c.enabled(),
                                type,
    ...
    private DictionaryAdminView byCode(String code) { // 私有方法：根据编码查详情
        return jdbc.query(
                "SELECT id,code,name,enabled,version FROM system_dictionary_type WHERE code=?",
                r -> { // 查询主表
    ...
    private DictionaryAdminView view(java.sql.ResultSet r) throws java.sql.SQLException { // 核心映射逻辑
        ...
        var items =
                jdbc.query(
                        "SELECT item_key,item_value,sort_order,enabled,version FROM system_dictionary_item WHERE type_id=? ORDER BY sort_order,item_key",
                        (i, n) ->
                                new DictionaryAdminView.Item(
                                        i.getString(1),
                                        i.getString(2),
                                        i.getInt(3),
                                        i.getBoolean(4),
        ...
        return new DictionaryAdminView(
                r.getString(2), r.getString(3), r.getBoolean(4), r.getLong(5), items); // 组装返回
    } // 结果
}
