...
public final class JdbcAuthorizationRepository
        implements AuthorizationRepository { // 定义最终类，实现领域层的授权仓储接口
    ...
    public AuthoritySnapshot snapshot(long user) { // 获取实现用户快照的方法，传入用户ID
        ...
        Long v =
                jdbc.query(
                        "SELECT version FROM system_user_authorization WHERE user_id=?",
        ...
        if (v == null) // 如果版本号为空，说明用户在授权表中尚未记录
            ...
            return new AuthoritySnapshot(
                    Long.toString(user),
                    Set.of("USER"),
                    Set.of("profile:self:read", "profile:self:write"),
                    0);
        // 查询用户当前用后的所有启用状态的角色编码，并按编码升序排序，放入 LinkedHashSet 中以保持顺序
        var roles =
                new LinkedHashSet<>(
                        jdbc.queryForList(
                                "SELECT r.code FROM system_user_role ur JOIN system_role r ON r.id=ur.role_id WHERE ur.user_id=? AND r.enabled=TRUE ORDER BY r.code",
                                String.class,
                                user));
    ...
    public Optional<AuthoritySnapshot> replaceRoles(
            ...
            Set<String> roles,
            long operator,
            String reason) { // 实现替换用户角色的方法（带事务和乐观锁）
        return tx.execute(
                s -> { // 启动事务执行块
                    ...
                    List<Map<String, Object>> found =
                            roles.isEmpty()
                                    ? List.of()
                                    : jdbc.queryForList(
                                            "SELECT id,code FROM system_role WHERE code IN ("
                                                    + String.join(
                                                            ",",
                    ...
                    jdbc.update(
                            "INSERT INTO system_user_authorization(user_id,version) VALUES(?,0) ON DUPLICATE KEY UPDATE user_id=VALUES(user_id)",
                            user);
                    // 第三步：通过 FOR UPDATE 语句对该用户授权进行排他锁定，并获取当前最新的版本号
                    Long current =
                            jdbc.query(
                                    "SELECT version FROM system_user_authorization WHERE user_id=? FOR UPDATE",
                                    r -> r.next() ? r.getLong(1) : null,
                                    user);
                    ...
                    for (var row : found)
                        jdbc.update(
                                "INSERT INTO system_user_role(user_id,role_id,assigned_by) VALUES(?,?,?)",
                                user,
                                ((Number) row.get("id")).longValue(),
                                operator);
                    // 第七步：更新用户授权版本号，使用其自增 1，完成乐观锁闭环
                    jdbc.update(
                            "UPDATE system_user_authorization SET version=version+1 WHERE user_id=?",
                            user);
                    // 第八步：插入授权审计日志，记录用户ID、操作人ID、旧角色集字符串、新角色集字符串以及变更原因
                    jdbc.update(
                            "INSERT INTO system_authorization_audit(user_id,operator_user_id,old_roles,new_roles,reason) VALUES(?,?,?,?,?)",
                            user,
                            operator,
                            String.join(",", new TreeSet<>(old)),
                            String.join(",", new TreeSet<>(roles)),
