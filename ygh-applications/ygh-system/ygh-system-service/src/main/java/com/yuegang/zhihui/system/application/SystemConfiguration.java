...
public class SystemConfiguration {
    ...
    @Bean
    // 注册 Bean：系统密文加密器
    SystemSecretCipher systemSecretCipher(
            @Value("${ygh.system.config-master-key-base64}") String encoded) {
    ...
    AiProviderConfigService aiProviderConfigService(
            DataSource dataSource, SystemSecretCipher secrets) {
    ...
    @Bean
    // 注册 Bean：系统字典管理服务
    SystemDictionaryAdministrationService systemDictionaryAdministrationService(
    ...
    SystemTrustedUserContextResolver systemTrustedUserContextResolver(
            @Value("${ygh.internal-request.hmac-base64}") String encoded, Clock clock) {
