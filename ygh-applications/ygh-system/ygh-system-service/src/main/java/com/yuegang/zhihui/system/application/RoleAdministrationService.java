...
public class RoleAdministrationService { // 定义最终类：角色管理服务
    ...
    private static long next() {
        return UUID.randomUUID().getMostSignificantBits()
                & Long.MAX_VALUE; //   1 / 生成 64 位随机数作为角色 ID
    } // 结束

    public List<RoleView> roles() { // 方法：查询所有角色列表
        return jdbc.query(
                "SELECT id, code, name, enabled, version FROM system_role ORDER BY built_in DESC, code",
                (r, n) ->
                        role(
                                r.getLong(1),
                                r.getString(2),
                                r.getString(3),
                                r.getBoolean(4),
                                r.getLong(5))); // 执行 SQL 查询并映射结果
    } // 结束

    public RoleView saveRole(String code, UpsertRoleRequest command) { // 方法: 保存或更新角色
        if (!code.equals(command.code()))
            throw new BusinessException(ErrorCode.VALIDATION_ERROR); // 校验路径编码与请求一致性
        return tx.execute(
                status -> { // 执行编排事务
                    Long id =
                            jdbc.query(
                    ...
                    if (id == null) { // 如果角色不存在，执行新增逻辑
                        if (command.version() != 0)
                            throw new BusinessException(ErrorCode.BUSINESS_CONFLICT); // 新增角色版本必须为 0
                        id = next(); // 生成新 ID
                        jdbc.update(
                    ...
                    throw new BusinessException(
                                ErrorCode.BUSINESS_CONFLICT); // 执行更新逻辑（含乐观锁），且禁止禁用内设超级管理员角色
        ...
        return jdbc.query(
                ...
                (r, n) ->
                        new PermissionView(
                                ...
                                r.getString(3),
                                r.getString(4),
                                r.getBoolean(5))); // 执行 SQL 查询并映射结果
    } // 结束

    public PermissionView savePermission(
            String code, UpsertPermissionRequest command) { // 方法: 保存/更新权限点
        if (!code.equals(command.code()))
            throw new BusinessException(ErrorCode.VALIDATION_ERROR); // 校验一致性
        jdbc.update(
                ...
                command.enabled()); // 执行插入或更新
        return jdbc.query(
                "SELECT id,code,name,resource_type,enabled FROM system_permission WHERE code=?",
                r -> { // 查询并返回最高权限对象
                    r.next();
    ...
    private RoleView getRole(long id) { // 私有方法：根据 ID 获取角色视图
        return jdbc.query(
                "SELECT id,code,name,enabled,version FROM system_role WHERE id=?",
                r -> { // 查询数据集
                    if (!r.next())
                        throw new BusinessException(ErrorCode.RESOURCE_NOT_FOUND); // 不存在则报错
                    return role(
                            r.getLong(1),
                            r.getString(2),
                            r.getString(3),
    ...
    private RoleView role(
            ...
            String name,
            boolean enabled,
            long version) { // 私有方法：深度组装角色（含权限点列表）
        Set<String> permissions =
                new LinkedHashSet<>(
                        jdbc.queryForList(
                                "SELECT p.code FROM system_permission p JOIN system_role_permission rp "
                                        + "ON p.id = rp.permission_id WHERE rp.role_id = ? ORDER BY p.code",
                                String.class,
                                id)); // 联表查询角色权限点列表
