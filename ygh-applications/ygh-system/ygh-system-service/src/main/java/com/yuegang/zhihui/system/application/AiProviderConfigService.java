...
public class AiProviderConfigService { // 定义最终类: AI 供应商配置服务
    ...
    private static AiProviderConfigView toView(
            String provider,
            String baseUrl,
            String chatModel,
        ...
        OffsetDateTime changed =
                updatedAt == null
                        ? null
                        : updatedAt.toInstant().atOffset(ZoneOffset.UTC); // 时间戳转 OffsetDateTime
    ...
    private static void validateBaseUrl(String value) {
        try { // 开启校验
            ...
            if (!"https".equalsIgnoreCase(uri.getScheme())
                    || uri.getHost() == null // 强制要求 HTTP 和主机名
                    || uri.getUserInfo() != null
                    || uri.getQuery() != null
                    || uri.getFragment() != null) { // 检查 URI 的各个部分是否合法
    ...
    private static String digest(String value) { // 私有静态方法：生成数据摘要
        try { // 开启哈希计算
            return HexFormat.of()
                    .formatHex(
                            MessageDigest.getInstance("SHA-256") // 使用 SHA-256 算法
    ...
    public AiProviderConfigView update(
            UpdateAiProviderConfigRequest request, long operator) { // 方法：更新 AI 配置
        ...
        if (request.apiKey() != null && !request.apiKey().isBlank()) { // 如果请求中携带了新的明文密钥
            ...
            if (normalized.length() < 16)
                throw new BusinessException(ErrorCode.VALIDATION_ERROR); // 密钥长度不足则抛错
        ...
        String newDigest =
                digest(
                        String.join(
                                ...
                                Boolean.toString(request.webSearchEnabled()),
                                oldCiphertext == null ? "" : oldCiphertext));
        // 将变更记录插入配置审计表
        jdbc.update(
                """
        ...
        return jdbc.queryForObject(
                ...
                (row, index) ->
                        toView( // 执行查询并转换为视图对象
                                ...
                                row.getString(2),
                                row.getString(3),
                                row.getString(4), // 映射供应商、URL、模型
                                row.getBoolean(5),
                                row.getString(6),
                                row.getLong(8),
                                row.getTimestamp(9) // 映射供应商、URL、模型
                                ));
    } // 方法结束
    ...
    public InternalAiProviderConfig internal() { // 方法：获取系统内部调用的配置（含解密后密钥）
        return jdbc.queryForObject(
                SELECT,
                (row, index) ->
                        new InternalAiProviderConfig(
                                row.getString(1),
                                row.getString(2),
                                row.getString(3),
                                row.getString(4), // 获取基础信息
                                row.getBoolean(5),
                                secrets.decrypt(row.getString(6), row.getString(7)), // 解密密钥
                                row.getLong(8) // 映射搜索开关，解密后的密码、版本
                                )); // 方法结束
    }
}
