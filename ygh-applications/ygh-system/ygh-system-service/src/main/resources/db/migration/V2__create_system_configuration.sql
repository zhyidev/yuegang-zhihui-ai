...
(
    ...
    version    BIGINT UNSIGNED                                   NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
    UNIQUE KEY uk_system_dictionary_item (type_id, item_key),
    CONSTRAINT fk_system_dictionary_item_type FOREIGN KEY (type_id) REFERENCES system_dictionary_type (id) ON DELETE CASCADE
);
...
CREATE TABLE system_feature_flag
(
    flag_key        VARCHAR(128) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
    ...
    CONSTRAINT chk_system_feature_rollout CHECK (rollout_percent <= 100)
);
CREATE TABLE system_configuration_audit
(
    id               BIGINT                                             NOT NULL,
    config_type      VARCHAR(24) CHARACTER SET ascii COLLATE ascii_bin  NOT NULL,
    config_key       VARCHAR(128) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
    old_digest       CHAR(64)                                           NULL,
    new_digest       CHAR(64)                                           NOT NULL,
    operator_user_id BIGINT                                             NOT NULL,
    created_at       DATETIME(6)                                        NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    PRIMARY KEY (id),
    INDEX idx_system_config_audit_key_time (config_type, config_key, created_at)
);
