...
(
    ...
    INDEX idx_ai_prompt_enabled (enabled)
);
CREATE TABLE ai_evaluation_case
(
    id                BIGINT                                            NOT NULL,
    category          VARCHAR(24) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
    question          TEXT                                              NOT NULL,
    expected_evidence TEXT                                              NOT NULL,
    forbidden_answer  TEXT                                              NULL,
    enabled           BOOLEAN                                           NOT NULL DEFAULT TRUE,
    created_by        BIGINT                                            NOT NULL,
    created_at        DATETIME(6)                                       NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    updated_at        DATETIME(6)                                       NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
    PRIMARY KEY (id),
    INDEX idx_ai_eval_category_enabled (category, enabled)
);
...
(
    ...
    created_at       DATETIME(6)     NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    PRIMARY KEY (id),
    INDEX idx_ai_eval_run_case_time (case_id, created_at)
);
