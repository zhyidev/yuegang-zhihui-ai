...
public final class MeasuredModelGateway implements ModelGateway {
    private final ModelGateway delegate;
    private final Timer duration;
    private final DistributionSummary promptTokens, completionTokens;

    public MeasuredModelGateway(ModelGateway d, MeterRegistry m) {
        delegate = d;
        duration = Timer.builder("ygh.ai.model.duration").description("Complete model response duration").register(m);
        promptTokens = DistributionSummary.builder("ygh.ai.prompt.tokens").register(m);
    ...
    public ModelAnswer answerWithSources(String system, String user) {
        promptTokens.record(tokens(system) + tokens(user));
        return duration.record(() -> {
            ModelAnswer answer = delegate.answerWithSources(system, user);
            completionTokens.record(tokens(answer.text()));
            return answer;
        });
    }
