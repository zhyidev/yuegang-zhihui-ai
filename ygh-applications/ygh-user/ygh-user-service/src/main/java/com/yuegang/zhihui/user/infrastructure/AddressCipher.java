 ...
 */
public final class AddressCipher { // 定义最终类 AddressCipher
    ...
    AddressCipher(String keyBase64, int keyVersion, SecureRandom random) {
        ...
        if (keyVersion < 1 || keyVersion > 65535)
            throw new IllegalArgumentException("PII key version is invalid"); // 校验版本范围
        this.key = new SecretKeySpec(decoded, "AES"); // 初始化 AES 密钥对象
    ...
    public byte[] encrypt(long userId, String field, String value) { // 加密方法：传入用户ID、字段名和明文
        ...
        try {
            ...
            cipher.updateAAD(aad(userId, field, keyVersion)); // 注入关联数据（AAD），防止密文在用户间或字段间被挪用
            // 分配缓冲区：长度 = IV长度 + 密文长度（含Tag），并依次存入数据
    ...
    public String decrypt(long userId, String field, int storedVersion, byte[] value) { // 解密方法
        ...
        try {
            ...
            cipher.init(Cipher.DECRYPT_MODE, key, new GCMParameterSpec(128, value, 0, IV_BYTES)); // 使用密文前缀作为 IV 初始化
            cipher.updateAAD(aad(userId, field, storedVersion)); // 注入相同的 AAD 进行完整性校验
