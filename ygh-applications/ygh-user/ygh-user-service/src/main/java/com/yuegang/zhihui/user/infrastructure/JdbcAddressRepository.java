...
public class JdbcAddressRepository implements AddressRepository { // 实现地址仓储接口
    ...
    public List<AddressView> findAll(long userId) { // 查找用户的所有地址
        ...
        return jdbc.query(
                SELECT + " WHERE user_id=? ORDER BY is_default DESC,updated_at DESC,id DESC",
                this::map,
                userId);
    }
    ...
    public AddressView create(long id, long userId, CreateAddressRequest r) { // 创建新地址
        return Objects.requireNonNull(
                tx.execute(
                        status -> { // 开启事务
                            boolean makeDefault =
                                    r.defaultAddress() || count(userId) == 0; // 如果用户指定或这是一条地址，则设为默认
                    ...
                    """,
                                    ...
                                    blankToNull(r.label()),
                                    cipher.encrypt(
                                            userId, "recipient_name", r.recipientName()), // 加密姓名
        ...
        return tx.execute(
                status -> { // 开启事务
                    Boolean currentlyDefault =
                            lockedDefault(id, userId, r.version()); // 锁定当前行并获取默认状态
                    if (currentlyDefault == null) return Optional.empty(); // 记录不存在返回空
                    if (r.defaultAddress() && !currentlyDefault)
                        clearDefault(userId); // 若想改为默认且原本不是，则清除旧默认
                    Boolean desireDefault =
                            r.defaultAddress()
                                    || (currentlyDefault && count(userId) == 1); // 维持唯一的默认状态
                    int changed =
                            jdbc.update(
                                    """
                    ...
                    """,
                                    ...
                                    r.districtName(),
                                    cipher.encrypt(userId, "addressDetail", r.addressDetail()),
                                    blankToNull(r.postalCode()),
                                    desireDefault,
                                    id,
                                    userId,
                                    r.version()); // 执行基于乐观锁 version 的更新
        ...
        return Boolean.TRUE.equals(
                tx.execute(
                        status -> { // 开启事务
                            Boolean wasDefault =
                                    jdbc.query(
                                            "SELECT is_default FROM user_address WHERE id=? AND user_id=? AND version=? FOR UPDATE",
                            ...
                            if (wasDefault == null
                                    || jdbc.update(
                                                    "DELETE FROM user_address WHERE id=? AND user_id=? AND version=?",
                                                    id,
                                                    userId,
                                                    version)
                                            != 1) return false; // 若不存在或删除失败，返回 false
    ...
    public Optional<AddressView> makeDefault(long id, long userId, long version) { // 手动设为默认
        return tx.execute(
                status -> {
                    ...
                    if (jdbc.update(
                                    "UPDATE user_address SET is_default=TRUE,version=version+1 WHERE id=? AND user_id=? AND version=?",
                                    id,
                                    userId,
                                    version)
                            != 1) return Optional.empty(); // 更新失败（乐观锁冲突）
    ...
    private void clearDefault(long userId) {
        jdbc.update(
                "UPDATE user_address SET is_default=FALSE, version=version+1 WHERE user_id=? AND is_default=TRUE",
                userId);
    } // 内部方法:清除用户所有默认标记
    ...
    private Optional<AddressView> one(long id, long userId) { // 内部方法：获取单条记录
        return jdbc.query(
                ...
                rs -> rs.next() ? Optional.of(map(rs, 1)) : Optional.empty(),
                id,
                userId);
    }
    ...
    private AddressView map(ResultSet rs, int row) throws SQLException { // 将数据库结果继续映射为视图模型
        ...
        return new AddressView(
                ...
                cipher.decrypt(
                        ...
                        "addressDetail",
                        keyVersion,
                        rs.getBytes("address_detail_ciphertext")), // 解密详细地址
                rs.getString("postal_code"),
                rs.getBoolean("is_default"),
                rs.getLong("version"),
                rs.getTimestamp("updated_at")
                        .toLocalDateTime()
                        .atOffset(ZoneOffset.UTC) // 按时间戳为 UTC OffsetDateTime
                );
    }
}
