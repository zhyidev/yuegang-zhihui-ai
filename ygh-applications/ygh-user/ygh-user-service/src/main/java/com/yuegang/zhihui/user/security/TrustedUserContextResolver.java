...
public final class TrustedUserContextResolver { // 定义受信任的内部用户上下文解析器类
    ...
    public CurrentUserPrincipal resolve(HttpServletRequest request) { // 解析请求中的用户信息
        try { // 开始尝试解析逻辑
            ...
            if (!signatures.verify(metadata, header(request, "X-YGH-User-Context-Signature"))) throw unauthenticated(); // 校验签名，若失败则
        ...
        }catch (BusinessException expected) { throw expected; } // 如果是已知的业务异常，直接向上抛出
        catch (RuntimeException malformed) { throw unauthenticated(); } // 如果发生运行时异常（如格式错误），视为认证失败
    }
