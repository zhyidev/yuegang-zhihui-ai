...
public class UserSecurityConfiguration { // 定义安全配置类
    ...
    TrustedUserContextResolver trustedUserContextResolver(
        ...
        try { secret = Base64.getDecoder().decode(encodedSecret); } // 解码 Base64 格式的密码
        catch (IllegalArgumentException malformed) { throw new IllegalStateException("internal request secret is malformed", malformed); } // 解码失败抛出异常
        try { return new TrustedUserContextResolver(secret, clock); } // 实例化解器
        finally { Arrays.fill(secret, (byte) 0); } // 安全擦除内存中的敏感密钥数据
    }

    @Bean // 将内部服务验证器注册为 Spring Bean
    UserInternalServiceVerifier userInternalServiceVerifier(
        ...
        try { return new UserInternalServiceVerifier(secret); } // 实例化验证器
        finally { Arrays.fill(secret, (byte) 0); } // 安全擦车内存中的敏感密钥数据
    }
}