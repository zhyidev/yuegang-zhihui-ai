...
class JdbcAddressRepositoryIntegrationTest {
    private static CreateAddressRequest create(String label, boolean preferred, String detail) {
        return new CreateAddressRequest(label, "张三", "13800138000", "CN", "440000", "广东省", "广州市", "天河区", detail, "510000", preferred);
    }

    private static UpdateAddressRequest update(AddressView a, long version) {
        return new UpdateAddressRequest(a.label(), a.recipientName(), a.recipientPhone(), a.countryCode(), a.provinceCode(), a.provinceName(), a.cityName(), a.districtName(), a.addressDetail(), a.postalCode(), false, version);
    }

    @Test
    void enforcesOwnershipOptimisticLockDefaultAndEncryptedStorage() throws Exception {
        try (var mysql = YghTestContainerFactory.mysql().start()) {
            ...
            try (var c = DriverManager.getConnection(mysql.jdbcUrl(), mysql.username(), mysql.credential()); var rs = c.createStatement().executeQuery("SELECT HEX(address_detail_ciphertext) value FROM user_address WHERE id=10")) {
                ...
                assertThat(rs.getString("value")).doesNotContain("天河路1号");
            }
        }
    }
}
