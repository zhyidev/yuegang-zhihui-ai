...
public final class OrderUserResolver {
    private final InternalUserContextSignature s;

    public OrderUserResolver(byte[] k) {
    ...
    public long resolve(HttpServletRequest r) {
        return context(r).user();
    }

    public long requirePermission(HttpServletRequest r, String permission) {
        Context c = context(r);
    ...
    private Context context(HttpServletRequest r) {
        try {
