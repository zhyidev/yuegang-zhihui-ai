...
(
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uk_order_cart_user_sku (user_id, sku_id),
    INDEX idx_order_cart_user_selected (user_id, selected),
    CONSTRAINT chk_order_cart_quantity CHECK (quantity > 0)
);
...
(
    ...
    INDEX idx_order_status_expiry (status, expires_at),
...
(
    ...
    CONSTRAINT fk_order_item_order FOREIGN KEY (order_id) REFERENCES order_main (id) ON DELETE RESTRICT,
    CONSTRAINT chk_order_item_amount CHECK (unit_price >= 0 AND quantity > 0 AND subtotal >= 0)
);
...
(
    ...
    CONSTRAINT fk_order_address_order FOREIGN KEY (order_id) REFERENCES order_main (id) ON DELETE RESTRICT
);
...
(
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uk_order_history_event (event_id),
    INDEX idx_order_history_order_time (order_id, occurred_at)
);
...
(
    ...
    PRIMARY KEY (id),
    INDEX idx_order_outbox_dispatch (status, next_retry_at)
);
