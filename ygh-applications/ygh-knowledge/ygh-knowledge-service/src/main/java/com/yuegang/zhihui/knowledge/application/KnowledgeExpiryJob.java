...
public final class KnowledgeExpiryJob {
    ...
    public void expire() {
        for (Long id : jdbc.queryForList("SELECT id FROM knowledge_document WHERE status='PUBLISHED' AND expires_at IS NOT NULL AND expires_at<=NOW(6) LIMIT 100", Long.class))
            tx.executeWithoutResult(s -> {
                if (jdbc.update("UPDATE knowledge_document SET status='EXPIRED',version=version+1 WHERE id=? AND status='PUBLISHED'", id) == 1) {
                    jdbc.update("INSERT INTO knowledge_status_history(id,document_id,from_status,to_status,operator_id,reason) VALUES(?,?,'PUBLISHED','EXPIRED',0,'POLICY_EXPIRED')", next(), id);
