...
public final class KnowledgeIndexJobController {
    ...
    @GetMapping
    ApiResponse<List<KnowledgeIndexJobView>> list(@RequestParam(required = false) String documentId, @RequestParam(required = false) String status, @RequestParam(defaultValue = "50") int limit, HttpServletRequest request) {
    ...
    @PostMapping("/rebuild")
    ApiResponse<RebuildKnowledgeIndexResponse> rebuild(@jakarta.validation.Valid @RequestBody RebuildKnowledgeIndexRequest command, HttpServletRequest request) {
        users.resolve(request, true);
