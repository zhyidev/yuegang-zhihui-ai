...
public class KnowledgeMetadataService {
    ...
    public KnowledgeMetadataView update(long operator, String document, UpdateKnowledgeMetadataRequest command) {
        long id = id(document);
        if (command.effectiveFrom() != null && command.expiresAt() != null && command.expiresAt().toLocalDate().isBefore(command.effectiveFrom())) {
        ...
        Map<String, Object> snapshot = Map.of("issuingAuthority", Objects.toString(command.issuingAuthority(), ""), "effectiveFrom", Objects.toString(command.effectiveFrom(), ""), "expiresAt", Objects.toString(command.expiresAt(), ""), "region", Objects.toString(command.region(), ""), "classification", command.classification(), "sourceName", Objects.toString(command.sourceName(), ""), "tags", command.tags());
        ...
        jdbc.update("INSERT INTO knowledge_document_version(id,document_id,version_no,title,metadata_json,content_sha256,created_by) VALUES(?,?,?,?,?,?,?)", next(), id, row.metadataVersion(), title, write(snapshot), sha, operator);
        return row;
    }
    ...
    public KnowledgeMetadataView view(long id) {
        return jdbc.query("""
            ...
            var tags = jdbc.queryForList("SELECT t.name FROM knowledge_tag t JOIN knowledge_document_tag dt ON dt.tag_id=t.id WHERE dt.document_id=? ORDER BY t.name", String.class, id);
            return new KnowledgeMetadataView(Long.toString(result.getLong(1)), result.getString(2), effective == null ? null : effective.toLocalDate(), expires == null ? null : expires.toInstant().atOffset(ZoneOffset.UTC), result.getString(5), result.getString(6), result.getString(7), tags, result.getLong(8));
        }, id);
    }
