...
public final class KnowledgeLifecycleService {
    ...
    public List<KnowledgeDocumentView> list(String status, String category, boolean publicOnly, int limit, Set<String> visibilities) {
        StringBuilder query = new StringBuilder("SELECT id,title,category,file_name,media_type,size_bytes,sha256,status,version,updated_at FROM knowledge_document WHERE 1=1");
        List<Object> arguments = new ArrayList<>();
        if (publicOnly) {
            if (visibilities.isEmpty()) return List.of();
            query.append(" AND status='PUBLISHED' AND (expires_at IS NULL OR expires_at>NOW(6)) AND visibility IN (").append(String.join(",", Collections.nCopies(visibilities.size(), "?"))).append(')');
    ...
    public KnowledgeDocumentView offline(long operator, String document, long version, String reason) {
        return transactions.execute(status -> {
            long id = id(document);
            if (jdbc.update("UPDATE knowledge_document SET status='OFFLINE',version=version+1 WHERE id=? AND version=? AND status='PUBLISHED'", id, version) != 1)
                throw new BusinessException(ErrorCode.BUSINESS_CONFLICT);
            jdbc.update("INSERT INTO knowledge_status_history(id,document_id,from_status,to_status,operator_id,reason) VALUES(?,?, 'PUBLISHED','OFFLINE',?,?)", next(), id, operator, reason);
            jdbc.update("INSERT INTO knowledge_index_job(id,document_id,index_version,job_type,status) VALUES(?,?,?,'DELETE','PENDING')", next(), id, "knowledge-v" + System.currentTimeMillis());
            return get(id);
        });
    }

    private KnowledgeDocumentView get(long id) {
        return jdbc.query("SELECT id,title,category,file_name,media_type,size_bytes,sha256,status,version,updated_at FROM knowledge_document WHERE id=?", result -> {
