...
public final class KnowledgeIndexJobService {
    ...
    public List<KnowledgeIndexJobView> list(String documentId, String status, int limit) {
        ...
        return jdbc.query(sql.toString(), (row, number) -> view(row.getLong(1), row.getLong(2), row.getString(3), row.getString(4), row.getString(5), row.getInt(6), row.getString(7), row.getTimestamp(8)), arguments.toArray());
    }
    ...
    public KnowledgeIndexJobView retry(String id) {
        ...
        return jdbc.query("SELECT id,document_id,index_version,job_type,status,retry_count,last_error,updated_at FROM knowledge_index_job WHERE id=?", result -> {
            if (!result.next()) throw new BusinessException(ErrorCode.RESOURCE_NOT_FOUND);
            return view(result.getLong(1), result.getLong(2), result.getString(3), result.getString(4), result.getString(5), result.getInt(6), result.getString(7), result.getTimestamp(8));
        }, jobId);
    }
