...
(
    ...
    PRIMARY KEY (sku_id),
    CONSTRAINT chk_inventory_quantities CHECK (available_quantity >= 0 AND locked_quantity >= 0 AND sold_quantity >= 0)
);
...
(
    ...
    updated_at     DATETIME(6)                                        NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
    PRIMARY KEY (id),
    UNIQUE KEY uk_inventory_reservation_request (request_id),
    INDEX idx_inventory_reservation_ref (reference_type, reference_id),
...
(
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uk_inventory_ledger_request_type (request_id, movement_type),
    INDEX idx_inventory_ledger_sku_time (sku_id, occurred_at)
);
...
(
    ...
    PRIMARY KEY (id),
    INDEX idx_inventory_reconciliation_status_time (status, checked_at)
);
