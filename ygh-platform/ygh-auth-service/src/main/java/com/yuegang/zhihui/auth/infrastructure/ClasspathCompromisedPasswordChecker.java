 ...
 */
public final class ClasspathCompromisedPasswordChecker
        implements CompromisedPasswordChecker { // 实现泄露密码检查接口

    public static final String DEFAULT_RESOURCE =
            "/security/common-passwords-sha256.bin"; // 默认资源文件路径
    ...
    public static final String EXPECTED_SHA256 =
            "d27cc6628a51c24255521284ce4c7d50bca1365a8224a42d0b92420df78c8ba6"; // 库文件本身的完整性校验哈希
    ...
    ClasspathCompromisedPasswordChecker(
            String resource, int expectedEntries, String expectedSha256) { // 全参构造函数
        Objects.requireNonNull(resource, "resource must not be null"); // 检查资源路径非空
        try (InputStream input =
                ClasspathCompromisedPasswordChecker.class.getResourceAsStream(
                        resource)) { // 获取资源输入流
            if (input == null) {
                throw new IllegalStateException(
                        "compromised-password dataset is unavailable"); // 抛出状态异常：数据集不可用
            }
            sortedDigests = input.readAllBytes(); // 读取所有字节到内存
        } catch (IOException readFailure) { // 捕获读取失败异常
            throw new IllegalStateException(
                    "compromised-password dataset cannot be read", readFailure); // 抛出状态异常，数据集无法读取
        }
        // 校验长度是否等于条数乘以每条长度，并校验文件内容的哈希值是否匹配
        if (sortedDigests.length != Math.multiplyExact(expectedEntries, DIGEST_LENGTH)
                || !MessageDigest.isEqual(
                        sha256(sortedDigests), HexFormat.of().parseHex(expectedSha256))) {
            Arrays.fill(sortedDigests, (byte) 0); // 校验失败，擦除内存数据
            throw new IllegalStateException(
                    "compromised-password dataset integrity check failed"); // 抛出异常: 完整性检验失败
        }
    }
    ...
    private static int compare(
            byte[] left, int leftOffset, byte[] right, int rightOffset) { // 比较两个字节数组切片的内部方法
        for (int index = 0; index < DIGEST_LENGTH; index++) { // 逐字节比较
            int comparison =
                    Integer.compare(
                            Byte.toUnsignedInt(left[leftOffset + index]),
                            Byte.toUnsignedInt(right[rightOffset + index]));
        ...
        try {
            bytes =
                    StandardCharsets.UTF_8
                            .newEncoder()
                            .encode(CharBuffer.wrap(normalized)); // 将字符转为UTF-8字符串
            ...
            while (low <= high) { // 执行二分查找
                ...
                int comparison =
                        compare(sortedDigests, middle * DIGEST_LENGTH, digest, 0); // 比较中间值与目标哈希
            ...
            throw new IllegalStateException(
                    "password cannot be UTF-8 encoded", impossibleUtf8Failure); // 抛出异常
