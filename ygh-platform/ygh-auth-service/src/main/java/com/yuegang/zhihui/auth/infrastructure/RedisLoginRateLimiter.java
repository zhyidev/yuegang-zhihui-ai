...
public class RedisLoginRateLimiter implements LoginRateLimiter { // 实现接口
    ...
    public LoginRateLimitDecision consume(String principal, InetAddress clientIp) { // 核心逻辑
        ...
        LoginRateLimitDimension dimension =
                !principalResult.allowed && !ipResult.allowed
                        ? LoginRateLimitDimension.BOTH
                        : !principalResult.allowed
                                ? LoginRateLimitDimension.PRINCIPAL
                                : LoginRateLimitDimension
                                        .IP; // Lua脚本: GET 获取值，若存在则删除，比较信号是否等于预期，返回 1(成功) 或
        // 0(失败/不存在)
        ...
        Duration retryAfter =
                switch (dimension) {
                    ...
                    case BOTH ->
                            principalResult.retryAfter.compareTo(ipResult.retryAfter) >= 0
                                    ? principalResult.retryAfter
                                    : ipResult.retryAfter;
                    case NONE ->
                            throw new IllegalStateException(
                                    "rejected rate-limit decision has no dimension");
                };
        // 返回被拒绝的决策
    ...
    private Result consumeOne(String key, int limit, Duration window) { // 执行单个 Redis 维度的计数操作
        List<?> raw =
                redis.execute(
                        ...
                        Long.toString(window.toMillis())); // 执行 Lua
        if (raw == null
                || raw.size() != 3
                || !(raw.get(0) instanceof Number allowed)
                || !(raw.get(1) instanceof Number ttl)) {
            throw new AccountSecurityPersistenceException(
                    "login rate-limit response is invalid", null); // 响应异常处理
        }
        // 返回内部结构：allowed 映射到长整型比较，retryAfter 映射到毫秒时长
        return new Result(
                allowed.longValue() == 1, Duration.ofMillis(Math.max(1, ttl.longValue())));
    }

    private record Result(boolean allowed, Duration retryAfter) {} // 内部临时记录
}
