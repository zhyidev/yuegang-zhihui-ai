...
public final class Argon2PasswordHasher { // 使用 Argon2id 算法的密码哈希器
    ...
    private static final Pattern ARGON2_ENCODING =
            Pattern.compile(
    ...
    public Argon2PasswordHasher( // 全参数构造函数
        ...
        if (saltLength != 16
                ...
                || memoryKiB < 8
                || memoryKiB > MAX_ACCEPTED_MEMORY_KIB
                || iterations < 1
    ...
    private ParsedEncoding parse(PasswordDigest digest) { // 解析编码字符串为内部对象
        if (digest == null
                || !ALGORITHM.equals(digest.algorithm().toUpperCase(Locale.ROOT))
        ...
        try {
            ...
            if (memory < 8
                    || memory > MAX_ACCEPTED_MEMORY_KIB
                    || time > MAX_ACCEPTED_ITERATIONS
    ...
    private <T> T withCapacity(java.util.function.Supplier<T> operation) { // 信号量保护包装器
        ...
        try {
            acquired =
                    capacity.tryAcquire(capacityWait.toMillis(), TimeUnit.MILLISECONDS); // 尝试获取令牌
    ...
    private record ParsedEncoding(
            int memoryKiB, int iterations, int parallelism, byte[] salt, byte[] hash) { // 内部解析数据载体
