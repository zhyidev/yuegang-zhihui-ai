...
import com.yuegang.zhihui.auth.infrastructure.*;
...
class AuthSecurityConfiguration {
    ...
    @Bean
    AccountAdministrationRepository accountAdministrationRepository(DataSource dataSource) {
        return new JdbcAccountAdministrationRepository(dataSource);
    }

    @Bean
    AccountAdministrationService accountAdministrationService(AccountAdministrationRepository repository, SessionStateStore sessions) {
        return new AccountAdministrationService(repository, sessions);
    }

    @Bean
    AuthAccountQueryService authAccountQueryService(DataSource dataSource) {
        return new AuthAccountQueryService(dataSource);
    }

    @Bean
    AuthTrustedUserContextResolver authTrustedUserContextResolver(
        ...
        try {
            return new AuthTrustedUserContextResolver(secret, clock);
        } finally {
    ...
    AuthorityProvider authorityProvider(
        ...
        byte[] secret = Base64.getDecoder().decode(encoded);
        try {
            return new HttpSystemAuthorityProvider(base, secret, clock);
        } finally {
            Arrays.fill(secret, (byte) 0);
        }
    }
    ...
    PasswordResetNotificationClient passwordResetNotificationClient(
        ...
        byte[] secret = Base64.getDecoder().decode(encoded);
        try {
            return new PasswordResetNotificationClient(baseUrl, secret, clock);
        } finally {
            Arrays.fill(secret, (byte) 0);
        }
    }
