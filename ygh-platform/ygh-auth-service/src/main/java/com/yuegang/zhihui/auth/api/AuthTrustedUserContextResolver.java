...
public final class AuthTrustedUserContextResolver {
    ...
    public AuthTrustedUserContextResolver(byte[] secret, Clock clock) {
        signatures = new InternalUserContextSignature(secret, clock, Duration.ofSeconds(30));
    }

    private static String required(HttpServletRequest r, String n) {
        String v = r.getHeader(n);
        if (v == null || v.isBlank()) throw failure();
        return v;
    }

    private static List<String> values(String v) {
        return v == null || v.isBlank() ? List.of() : List.of(v.split(",", -1));
    }
    ...
    public CurrentUserPrincipal resolve(HttpServletRequest request) {
        try {
            String user = required(request, "X-YGH-User-Id");
            var roles = values(request.getHeader("X-YGH-Roles"));
            var permissions = values(request.getHeader("X-YGH-Permissions"));
            ...
            return new CurrentUserPrincipal(user, new LinkedHashSet<>(roles), new LinkedHashSet<>(permissions));
