...
public record ApiResponse<T>(     // 使用Java Record定义响应时间 3 usages
                                  String code,              // 状态业务码 1 usage
                                  String message,          // 提示信息 1 usage
                                  T data,                  // 泛型数据负载 no usages
                                  String traceId,          // 请求链路追踪 ID 1 usage
                                  OffsetDateTime timestamp) { // 响应生成的时间戳 1 usage
    public ApiResponse { // 紧凑构造函数，用于参数校验 1 usage
    ...
    public static <T> ApiResponse<T> success(T data, String traceId) { // 静态快捷成功方法 no usages
    ...
    public static <T> ApiResponse<T> failure(ErrorCode errorCode, String message, String traceId) { // 简单失败方法 no usages
        return failure(errorCode, message, null, traceId); // 调用全参数失败方法，数据传空
    }

    public static <T> ApiResponse<T> failure( // 完整参数失败方法 1 usage
