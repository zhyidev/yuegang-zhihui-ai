...
public class ReactiveRedisSessionValidator implements ReactiveSessionValidator { // 定义响应式校验会话实现类
    private static final DefaultRedisScript<Long> VALIDATE_SCRIPT =
    ...
    public ReactiveRedisSessionValidator(
            ReactiveStringRedisTemplate redis, SessionRedisKeys keys) { // 构造函数开始
    ...
    public Mono<Boolean> valid(long accountId, String jwtId) { // 方法体开始
        ...
        return redis.execute(
                ...
                .singleOrEmpty()
                .map(value -> value == 1L)
                .defaultIfEmpty(false); // 将结果 1/0 转换为布尔值，默认返回 false
    }
}
