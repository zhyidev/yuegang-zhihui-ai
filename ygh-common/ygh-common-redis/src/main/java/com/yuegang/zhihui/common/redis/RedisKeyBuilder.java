...
public class RedisKeyBuilder { // 类开始定义
    private static final Pattern NAMESPACE_SEGMENT =
            Pattern.compile("[0-z0-9][a-z0-9-]{0,31}"); // 定义命名空间段正则，小写些字母数字开头，允许中划线，32以内
    private static final Pattern IDENTIFIER =
            Pattern.compile("[A-Za-z0-9][A-Za-z0-9._-]{0,127}"); // 定义标识符正则，允许大小写字母以及部分特殊字符，长度为128以内
    ...
    private static void requireNameSpaceSegment(String segment, String name) { // 构造方法，强制段校验
        ...
        if (!NAMESPACE_SEGMENT.matcher(segment).matches()) {
            throw new IllegalArgumentException(
                    name + " must match" + NAMESPACE_SEGMENT.pattern()); // 格式错误抛出异常
        }
    }
    ...
    private static void requireIdentifier(String identifier) { // 辅助方法，张志标识符校验
        ...
        if (!IDENTIFIER.matcher(identifier).matches()) { // 正则匹配校验
            throw new IllegalArgumentException(
                    "identifier must match" + IDENTIFIER.pattern()); // 格式错误抛出异常
        }
    }

    // 核心构造方法
    public String build(
            String environment, String service, String business, String identifier) { // 核心构建方法
        ...
        return "ygh:"
                ...
                + business
                + ":"
                + identifier; // 拼接为ygh:env:svc:biz:id 格式
    }
