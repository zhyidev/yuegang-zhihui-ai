...
public class TtlJitterPolicy {
    ...
    public Duration cacheTtl(Duration baseTtl) { // 计算方法开始
        ...
        long offset =
                -windowMillis
                        + randomOffset(
                                Math.multiplyExact(
                                        windowMillis, 2L)); // 生成 [-window, window] 之间的偏移量
        return Duration.ofMillis(
                Math.max(1L, Math.addExact(baseMillis, offset))); // 返回最终的Duration ，最低保证 1ms
    }
    ...
    public Duration minimumRetentionTtl(Duration minttl) { //
        ...
        return Duration.ofMillis(
                Math.addExact(
                        baseMillis, randomOffset(windowMillis))); // 返回最终的 Duration ，保证最低不低于传入的最小值
    }
    ...
    public long requireBaseTtl(Duration baseTtl) { // 基准时间校验
        ...
        if (baseTtl.compareTo(Duration.ofMillis(1)) < 0 // 必须大于 1ms
            ...
            throw new IllegalArgumentException(
                    "baseTtl must be between 1ms and " + MAX_BASE_TTL.toDays() + " days");
        }
        return baseTtl.toMillis(); // 返回毫秒值
    }

    private long jitterWindowMillis(long baseMillis) {
        return Math.round(Math.multiplyExact(baseMillis, 10_000L) * ratio / 10_000.0d); // 计算抖动窗口大小
    }
