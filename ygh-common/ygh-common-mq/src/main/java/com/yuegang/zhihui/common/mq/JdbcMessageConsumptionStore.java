...
public class JdbcMessageConsumptionStore implements MessageConsumptionStore {
        ...
        this.transaction =
                new TransactionTemplate(
                        Objects.requireNonNull(
                                transactionManager,
                                "transactionManager must not be null")); // 校验并根据事务管理器创建事务模板
        this.clock = Objects.requireNonNull(clock, "clock must not be null"); // 校验并注入时钟
        this.duplicateClaimObserver =
                Objects.requireNonNull(
                        duplicateClaimObserver,
                        "duplicateClaimObserver must not be null"); // 校验并注入观察者
    }
    ...
    private static boolean isCurrentClaim(
            ...
            Instant now) { // 必须满足：有数据、状态是处理中、持有者一致、租约过期
        return row != null
    ...
    private static void requireLease(Duration lease) {
        ...
        if (lease.compareTo(Duration.ofSeconds(1)) < 0
            ...
            throw new IllegalArgumentException(
                    "lease must be between 1 second and 15 minutes"); // 抛出异常
        }
    }

    @Override
    public MessageClaimResult claim(
            String consumerGroup, // 尝试认领消息处理权的方法
            String eventId, // 组名
            String owner, // 事件 ID
            Duration lease) { // 租约时长
        requireLease(lease); // 校验租约合法性
        MessageProcessingClaim claim =
                new MessageProcessingClaim(consumerGroup, eventId, owner); // 构造凭证对象
        ...
        try { // 开启计算块
            ...
            throw new IllegalArgumentException(
                    "lease exceeds the supported time range", exception); // 抛出参数非法异常
        }
        try { // 开启事务执行块
            MessageClaimResult result =
                    transaction.execute(status -> doClaim(claim, now, leaseUntil)); // 在事务中执行实际的认领逻辑
                    ...
                    transaction.execute(
                            status -> { // 执行编辑排逻辑
                                ConsumptionRow row =
                                        lockRow(
                                                claim.consumerGroup(),
                                                claim.eventId()); // 1. 使用 SELECT FOR UPDATE 锁住记录
                                if (!isCurrentClaim(
                                        row, claim, clock.instant())) { // 2. 检查当前认领是否任然归属当前节点并且未超时
                                ...
                                if (changed != 1) { // 如果没有更新成功 理论上受事务保护不应发生
                                    throw new IllegalArgumentException(
                                            "claim is no longer valid, cannot mark success"); // 抛出异常
                                }
                                return true; // 流程成功
                            }); // 提交事物点
        ...
        try { // 执行删除操作
            return jdbc.update( // 直接删除 PROCESSING 记录，中间件重复投递时间可重复认领
                            "DELETE FROM mq_consumption WHERE consumer_group = ? AND event_id = ?"
                                    + " AND status = ? AND owner = ?", // 使用乐观锁 + 条件删除
                            claim.consumerGroup(),
                            claim.eventId(),
                            PROCESSING,
                            claim.owner())
    ...
    public boolean markDeadLettered(MessageProcessingClaim claim, DeadLetterRecord record) {
        ...
        if (!claim.consumerGroup().equals(record.customerGroup())
                || !claim.eventId().equals(record.eventId())) { // 检验死信是否和当前 ID 匹配
        ...
        try { // 开启事务块
            Boolean completed =
                    transaction.execute(
                            status -> { // 执行数据库事务
                                ConsumptionRow row =
                                        lockRow(claim.consumerGroup(), claim.eventId()); // 加锁获得当前行
                                ...
                                int changed =
                                        jdbc.update( // 2. 更新主表状态为 DEAD_LETTERED
                                                "UPDATE mq_consumption SET status = ?, owner = NULL, lease_until=NULL,"
                                                        + "updated_at = ? WHERE consumer_group = ? AND event_id = ?"
                                ...
                                if (changed != 1) { // 状态检查
                                    throw new IllegalArgumentException(
                                            "claim changed inside locked transaction");
                                }
                                return true; // 完成入库
                            }); // 结束事务
    ...
    private MessageClaimResult doClaim(
            MessageProcessingClaim claim, Instant now, Instant leaseUntil) {
        try { // 尝试插入数据库认领
            ...
            duplicateClaimObserver.afterDuplicate(
                    claim.consumerGroup(), claim.eventId()); // 查询当前数据库行信息
            ...
            if (SUCCEEDED.equals(existing.status())
                    || DEAD_LETTERED.equals(existing.status())) { // 如果状态是成功的或死信的
            ...
            int changed =
                    jdbc.update( // 。场景，前一个认领者的租约已超时，当前节点可以强制抢占处理权
    ...
    private ConsumptionRow lockRow(String consumerGroup, String eventId) {
        return jdbc.query( // 查询加锁
                "SELECT status,owner,lease_until FROM mq_consumption "
                        + "WHERE consumer_group = ? AND event_id = ? FOR UPDATE", // FOR update
                // 触发数据库行锁
                resultSet ->
                        resultSet.next() // 处理结果
                                ? new ConsumptionRow(
                                        resultSet.getString("status"), // 获取状态
                                        resultSet.getString("owner"), // 获取持有者
                                        toInstant(
                                                resultSet.getTimestamp("lease_until"))) // 获取租约到期时间
                                : null,
                consumerGroup,
                eventId);
    }
