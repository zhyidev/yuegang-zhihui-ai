...
public final class FlywayMigrationPolicy { // 强制执行脚本命名和内容规范
    ...
    private static final Pattern VERSIONED_MIGRATION =
            Pattern.compile("^db/migration/V([1-9][0-9]*)__([a-z][a-z0-9]*(?:_[a-z0-9]+)*)\\.sql$");
    ...
    public static MigrationPolicyException invalid(String path) { // 命名错误工厂
        ...
        return new MigrationPolicyException(
                ...
                "migration name must match db/migration/V<positive integer>_<lower snake case>.sql: "
                        + displayPath);
    }
    ...
    public MigrationDescriptor migrationDescriptor(String resourcePath) { // 解析脚本路径为描述对象
        ...
        if (normalized.isBlank()
                || normalized.contains("../")
                || normalized.contains("/../")) { // 禁止路径穿越攻击
            throw invalid(normalized); // 路径非法按命名不合规处理
        }
        if (normalized.startsWith("db/migration/U")) { // 禁止 Undo 脚本，保证迁移不可回退
    ...
    public MigrationValidationReport validate(Collection<String> resourcePaths) { // 批量验证脚本列表
        ...
        for (String resourcePath : resourcePaths) { // 遍历所有路径
            ...
            try {
                ...
                String existingPath =
                        pathByVersion.putIfAbsent(
                                descriptor.version(), descriptor.resourcePath()); // 检查版本号是否重复
                ...
                violations.add(
                        new MigrationViolation( // 命名不合规同样记入报告
                                exception.getCode(),
    ...
    public MigrationValidationReport validateNewMigrations(
            Collection<String> resourcePaths, long highestAppliedVersion) { // 验证新脚本是否满足
        if (highestAppliedVersion < 0) {
            throw new IllegalArgumentException("highestAppliedVersion must not be negative");
        }
        var violations =
                new ArrayList<>(validate(resourcePaths).migrationViolationlist()); // 先执行基础静态验证
                ...
                if (descriptor.version() <= highestAppliedVersion) { // 如果版本号小于等于已应用的最高版本号，则违规
                    violations.add(
                            new MigrationViolation(
                                    ...
                                    "Migration version "
                                            + descriptor.version()
                                            + " is not greater than highest applied version "
