...
public class YghFlywayMigrationStrategy implements FlywayMigrationStrategy { // 拦截 Spring 自动执行的迁移
    ...
    public YghFlywayMigrationStrategy(
        ...
        this.migrationPolicy =
                Objects.requireNonNull(migrationPolicy, "migrationPolicy must not be null");
        this.configurationGuard =
                Objects.requireNonNull(configurationGuard, "configurationGuard must not be null");
        this.historyValidator =
                Objects.requireNonNull(historyValidator, "historyValidator must not be null");
    }

    // --- 内部辅助判断方法 ---
    private static boolean isRepeatable(InfoOutput migration) {
        boolean repeatableCategory =
                migration.category != null
                        && migration.category.toLowerCase().contains("repeatable");
        boolean SqlWithoutVersion =
                resolvedVersion(migration) == null
    ...
    private static boolean isApplied(InfoOutput migration) {
        ...
        return migration.state != null && migration.state.equalsIgnoreCase("Success");
    }
    ...
    public void migrate(Flyway flyway) { // 核心迁移，逻辑拦截
        ...
        for (InfoOutput migration : info.migrations) { // 迭代所有脚本
            ...
            if (migration.filepath != null && migration.filepath.endsWith(".sql")) { // 只处理 SQL 脚本
                String policyPath =
                        configurationGuard.toPolicyResourcePath(
                                flyway.getConfiguration(), migration.filepath);
                allResources.add(policyPath);
                if (!isApplied(migration)) { // 如果尚未安装，假如待执行列表
            ...
            if (resolvedVersion != null && isApplied(migration)) { // 计算历史最高版本号
                highestAppliedVersion =
                        Math.max(highestAppliedVersion, parseAppliedVersion(resolvedVersion));
            }
        }
        migrationPolicy.validate(allResources).throwIfInvalid(); // 2.执行静态规范验证
        migrationPolicy
                .validateNewMigrations(notAppliedResources, highestAppliedVersion)
                .throwIfInvalid(); // 4. 检验脚本是否合法
