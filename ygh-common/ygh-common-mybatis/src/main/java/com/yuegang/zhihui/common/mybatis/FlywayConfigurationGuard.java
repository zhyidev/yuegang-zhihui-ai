...
public class FlywayConfigurationGuard { // 防止不合规的 Flyway 配置被应用
    ...
    public FlywayConfigurationGuard(Set<String> approvedLocations) { // 自定义核准路径构造
        ...
        this.approvedLocations =
                approvedLocations.stream() // 规范化路径描述符
                        .map(Location::new)
    ...
    public void validateOrThrow(Configuration configuration) { // 核心校验方法
        ...
        Set<String> configurationLocation =
                Set.of(configuration.getLocations()).stream() // 获取当前实际配置路径
                        .map(Location::getDescriptor)
                        .collect(Collectors.toUnmodifiableSet());
        // 强制安全性校验项
        boolean safe =
                configuration.isValidateMigrationNaming() // 必须开启命名校验
                        ...
                        && approvedLocations.equals(configurationLocation); // 路径必须匹配核准列表
        if (!safe) {
