...
public class GlobalExceptionHandler { // 类定义
    ...
    public ResponseEntity<ApiResponse<Void>> handleBusinessException( // 处理函数
        ...
        var body =
                ApiResponse.<Void>failure( // 构建失败响应信封
                        exception.errorCode(),
                        message,
                        TraceIdResolver.resolve(request)); // 注入 Trace ID
    ...
    public ResponseEntity<ApiResponse<List<FieldValidationError>>>
            handleMethodArgumentNotValid( // 处理函数
                    MethodArgumentNotValidException exception, // 参数
                    HttpServletRequest request // 请求参数
                    ) {
        List<FieldValidationError> fieldErrors =
                exception.getBindingResult().getFieldErrors().stream() // 处理错误流
                        .map(
                                error ->
                                        FieldValidationError.sanitized( // 将错误清洗映射
    ...
    public ResponseEntity<ApiResponse<List<FieldValidationError>>>
            handleHandlerMethodValidation( // 函数定义
                    HandlerMethodValidationException exception, // 参数
                    HttpServletRequest request // 参数
                    ) {
        List<FieldValidationError> fieldErrors =
                exception.getParameterValidationResults().stream() // 处理结果数据流参数结果流
                        .flatMap(
                                result ->
                                        result.getResolvableErrors().stream() // 打平错误信息
                                                .map(
                                                        error ->
                                                                FieldValidationError
                                                                        .sanitized( // 映射清洗
                                                                                resolveParameterName(
                                                                                        result
                                                                                                .getMethodParameter()), // 获取参数名
                                                                                resolveValidationMessage(
                                                                                        error
                                                                                                .getDefaultMessage()) // 解析信息
                                                                                )))
                        .toList(); // 集合化
    ...
    public ResponseEntity<ApiResponse<List<FieldValidationError>>> handleConstraintViolation(
        ...
        List<FieldValidationError> fieldErrors =
                exception.getConstraintViolations().stream() // 处理约束违反
                        .map(
                                violation ->
                                        FieldValidationError.sanitized( // 映射清洗
                                                violation.getPropertyPath().toString(), // 获取字段名
                                                resolveValidationMessage(
                                                        violation.getMessage()) // 解析信息
                                                ))
                        .toList(); // 集合化
        return validationFailure(fieldErrors, request); // 统一调用校验失败构建器
    }
    ...
    @ExceptionHandler(HttpMediaTypeNotSupportedException.class) // 处理 Content-Type 格式不对
    ...
    public ResponseEntity<ApiResponse<List<FieldValidationError>>>
            handleMediaTypeNotAcceptable( // 定义
    ...
    public ResponseEntity<ApiResponse<Void>> handleResourceNotFound( // 定义
        ...
        var body =
                ApiResponse.<Void>failure( // 构建失败响应
        ...
        return ResponseEntity.status(errorResource.getStatusCode()) // 设置 HTTP 状态码
                .headers(errorResource.getHeaders())
                .body(body); // 返回响应实体
    }
    ...
    // 处理参数缺失或类型不对
    public ResponseEntity<ApiResponse<List<FieldValidationError>>>
            handleRequestBindingFailure( // 定义
                    Exception exception, // 参数
                    HttpServletRequest request // 请求参数
                    ) {
        return validationFailure( // 返回格式化错误
    ...
    public ResponseEntity<ApiResponse<Void>> handleUnexpectedException( // 定义
        ...
        LOGGER.error(
                "Unexpected exception, traceId="
                        + traceId
                        + ", type="
                        + exception.getClass().getName()); // 打印类名
        var body =
                ApiResponse.<Void>failure( // 返回前端 500 错误
    ...
    private String resolveParameterName(
            org.springframework.core.MethodParameter parameter) { // 参数名解析
