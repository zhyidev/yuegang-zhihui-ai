...
public class YghOpenApiAutoConfiguration { // 定义类
    ...
    private Schema<?> apiResponseSchema() { // 定义统一响应的结构
        Schema<?> schema =
                new ObjectSchema() // 定义为对象
                        .addProperty("code", new StringSchema().description("稳定业务错误码")) // 定义属性 code
                        .addProperty(
                                "message",
                                new StringSchema().description("可安全展示的消息")) // 定义属性 message
                        .addProperty(
                                "data", new Schema<>().description("业务数据：失败时通常为空")) // 定义属性 data
                        .addProperty(
                                "traceId", new StringSchema().description("链路追踪标识")) // 定义属性 traceId
                        .addProperty(
                                "timestamp",
                                new StringSchema().format("date-time")); // 定义属性 timestamp
                        ...
                        .addProperty(
                                ...
                                new ArraySchema()
                                        .items( // 定义 data 为数组类型
                                                new Schema<>()
                                                        .$ref(
                                                                "#/components/schemas/FieldValidationError"))) // 数组内元素引用上面的条目模型
    ...
    private void addErrorResponse( // 辅助方法，快速添加 HTTP 响应模板
        ...
        var mediaType =
                new io.swagger.v3.oas.models.media.MediaType() // 定义媒体类型：JSON
                        .schema(
                                new Schema<>()
                                        .$ref("#/components/schemas/" + schemaName)); // 引用结构类型
        components.addResponses(
                name,
                new ApiResponse() // 向文档库注册一种响应类型
                        .description(description) // 设置文字描述
                        .content(
                                new io.swagger.v3.oas.models.media.Content() // 设置返回内容
                                        .addMediaType(
                                                "application/json", mediaType))); // 绑定 JSON 映射
    }
