...
public class InternalServicesSignature {
    public final class InternalServiceSignature { // 定义签名类 no usages
        ...
        public record Metadata(String service, String method, String path, Instant timestamp) { // 数据对象定义 3 usages
            public Metadata { // 数据校验 no usages
                if (service == null || !service.matches("[a-z0-9-]{2,63}")) throw new IllegalArgumentException("unsafe service"); // 校验服务
                if (method == null || !method.matches("[A-Z]{3,10}")) throw new IllegalArgumentException("unsafe method"); // 校验方法
