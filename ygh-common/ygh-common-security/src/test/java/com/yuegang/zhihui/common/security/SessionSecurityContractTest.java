...
class SessionSecurityContractTest { // 会话安全契约（SessionSecurtiy/SessionRevocationStore) 单元测试类 ）
    @Test
    // 标记为 JUnit5 测试方法
    void shouldAllowInfrastructureToProvideRevocationAndAccountState() { // 测试方法：
        // 验证允许基础设置层提供会话撤销与账号启用状态绑定
        SessionRevocationStore revocations = new FakeRevocationStore(); // 实例化测试用得伪会话撤销存储
        AccountStatusProvider accountStatusProvider =
                userId ->
                        !"disabled-user"
                                .equals(userId); // 使用 Lambda 实现账号状态提供者，即disabled—-user 外部提供者
        ...
        assertThat(
                        revocations.isUserSessionRevoked( // 验证 user-1 在07:59:59 （遭遇截止时间）签发会话已被取消
                                "user-1", // 用户ID
    ...
    private static final class FakeRevocationStore
            implements SessionRevocationStore { // 静态私有内存假对象，模拟会话册小存储基础设施
        ...
        @Override
        public void revokeUserSessionsIssuedBefore(
                String userId, Instant issuedBefore) { // 批量撤销早于指定时间的公告
        ...
        public boolean isUserSessionRevoked(String userId, Instant issuedAt) { // 检查用户指定的签发时间会话已被撤销
            return userId.equals(revokedUser)
                    && issuedAt.isBefore(revokeBefore); // 匹配已经记录的 Token ，如果是该用户且签发时间早于撤销截止时间
        }
    }
}
